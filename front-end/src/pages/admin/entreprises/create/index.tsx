...
const CreateEntreprise: React.FC<CreateEntrepriseProps> = ({
  isEditMode,
  existingEntreprise,
}) => {
  ...
  const handleLocaliteInputChange = (
  ...
  const handleRemoveLocalite = (localiteId: number) => {
    ...
    axios
      ...
      .then(() => {
        ...
        setEntreprise((prevData) => ({
          ...prevData,
          localities: prevData.localities.filter(
            (localities) => localities.id !== localiteId
          ),
        }));
      })
      .catch((error) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditMode) {
      ...
      axios
        .put(
          `http://localhost:8000/api/entreprise/${entreprise.id}`,
          entrepriseData,
          config
        )
        .then(() => {
      ...
      axios
        .post("http://localhost:8000/api/entreprise", entrepriseData, config)
        .then(() => {
